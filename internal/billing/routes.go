...
func RegisterRoutes(api fiber.Router) {
	...
	send := middleware.RequirePermission("billing.send")
	creditNote := middleware.RequirePermission("billing.credit_note")
	...
	api.Post("/billing/send/:saleId", mod, send, h.SendToSUNAT)
	api.Get("/billing/status/:saleId", mod, send, h.GetBillingStatus)
	api.Get("/billing/events", mod, send, handler.SSEAccessTokenMiddleware, h.BillingEventsSSE)
	api.Get("/billing/job/:saleId", mod, send, h.GetBillingJobStatus)
	api.Post("/billing/resend/:saleId", mod, send, h.ResendToSUNAT)
	...
	api.Post("/billing/void-with-credit-note/:saleId", mod, creditNote, h.VoidWithCreditNoteAPI)
	...
	api.Get("/billing/invoice/:saleId", mod, send, h.GetInvoiceAPI)
	api.Get("/billing/invoice/:saleId/document/:kind", mod, send, h.GetInvoiceDocumentAPI)
